...
void CGameFramework::CreateSwapChain()
{
	...
	dxgiSwapChainDesc.Height = m_nWndClientHeight; // 높이
	dxgiSwapChainDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // 픽셀 형식
	dxgiSwapChainDesc.SampleDesc.Count = (m_bMsaa4xEnable) ? 4 : 1; // 다중표본화 표본 개수
	dxgiSwapChainDesc.SampleDesc.Quality = (m_bMsaa4xEnable) ? (m_nMsaa4xQualityLevels - 1) : 0; // 다중표본화 품잘 수준 
	dxgiSwapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT; // 후면 버퍼에 렌더링한다.
	dxgiSwapChainDesc.BufferCount = m_nSwapChainBuffers; // 교환 사슬이 사용할 버퍼 개수
...
{
	// 서술자 힙: 서술자뷰를 담는 곳 RTV, DSV 생성

	D3D12_DESCRIPTOR_HEAP_DESC d3dDescriptorHeapDesc;
	::ZeroMemory(&d3dDescriptorHeapDesc, sizeof(D3D12_DESCRIPTOR_HEAP_DESC));
	d3dDescriptorHeapDesc.NumDescriptors = m_nSwapChainBuffers; // 개수 설정. 렌더 타겟 만큼 생성
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	// 파이프라인 상태 객체 매개변수에 널 포인터를 지정했음.
	// -> 어떤 그리기 명령도 제출하지 않았기 때문
...
void CGameFramework::CreateRenderTargetViews()
{
	...
	// 자원 자체를 직접 파이프라인의 단계에 묶지 않음. 반드시 자원에 대한 뷰를 생성해서 그 뷰를 파이프라인 단계에 묶어야 함.
