...
void CSEdgeDetection(int3 n3GroupThreadID : SV_GroupThreadID, int3 n3DispatchThreadID : SV_DispatchThreadID)
{
	...
	else if ((n3GroupThreadID.x == 31) && (n3GroupThreadID.y == 0)) gf4GroupSharedCache[33][0] = gtxtInput[int2(n3DispatchThreadID.x+1, n3DispatchThreadID.y-1)];
	else if ((n3GroupThreadID.x == 0) && (n3GroupThreadID.y == 31)) gf4GroupSharedCache[0][33] = gtxtInput[int2(n3DispatchThreadID.x-1, n3DispatchThreadID.y+1)];
	else if ((n3GroupThreadID.x == 31) && (n3GroupThreadID.y == 31)) gf4GroupSharedCache[33][33] = gtxtInput[int2(n3DispatchThreadID.x+1, n3DispatchThreadID.y+1)];
	...
	if (n3GroupThreadID.x == 31) gf4GroupSharedCache[33][n3GroupThreadID.y+1] = gtxtInput[int2(n3DispatchThreadID.x+1, n3DispatchThreadID.y)];
	if (n3GroupThreadID.y == 31) gf4GroupSharedCache[n3GroupThreadID.x+1][33] = gtxtInput[int2(n3DispatchThreadID.x, n3DispatchThreadID.y+1)];
