...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pTexture = new CTexture(3, RESOURCE_TEXTURE2D, 0, 1, 1, 1);
	...
	m_pTexture->CreateTexture(pd3dDevice, 1024, 1024, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON, NULL, RESOURCE_TEXTURE2D, 2);

	CreateShaderResourceViews(pd3dDevice, m_pTexture, 0);
	CreateUnorderedAccessView(pd3dDevice, m_pTexture, 2);

	m_pTexture->SetComputeSrvRootParameterIndex(0, 0, 0);
	m_pTexture->SetComputeUavRootParameterIndex(0, 1, 2);

	m_pTexture->SetGraphicsSrvRootParameterIndex(0, 0, 2);

	m_nComputeShaders = 1;
	m_ppComputeShaders = new CComputeShader*[m_nComputeShaders];

	CTextureAddComputeShader *pComputeShader = new CTextureAddComputeShader();
	pComputeShader->CreateShader(pd3dDevice, m_pd3dComputeRootSignature, 32, 32, 1);
...
void CScene::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	::SynchronizeResourceTransition(pd3dCommandList, m_pTexture->GetResource(2), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
	...
	::SynchronizeResourceTransition(pd3dCommandList, m_pTexture->GetResource(2), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON);
