...
inline UINT8 D3D12GetFormatPlaneCount(
    _In_ ID3D12Device* m_pd3dDevice,
    DXGI_FORMAT Format
    )
{
...
struct CD3DX12_RESOURCE_DESC : public D3D12_RESOURCE_DESC
{
    ...
    { return (Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE3D ? DepthOrArraySize : 1); }
    inline UINT8 PlaneCount(_In_ ID3D12Device* m_pd3dDevice) const
    { return D3D12GetFormatPlaneCount(m_pd3dDevice, Format); }
    inline UINT Subresources(_In_ ID3D12Device* m_pd3dDevice) const
    { return MipLevels * ArraySize() * PlaneCount(m_pd3dDevice); }
    ...
    _In_range_(0,D3D12_REQ_SUBRESOURCES-FirstSubresource) UINT NumSubresources)
{
    ...
    pDestinationResource->GetDevice(__uuidof(*m_pd3dDevice), reinterpret_cast<void**>(&m_pd3dDevice));
    m_pd3dDevice->GetCopyableFootprints(&Desc, FirstSubresource, NumSubresources, 0, nullptr, nullptr, nullptr, &RequiredSize);
    ...
    _In_reads_(NumSubresources) D3D12_SUBRESOURCE_DATA* pSrcData)
{
    ...
    ID3D12Device* m_pd3dDevice = nullptr;
    pDestinationResource->GetDevice(__uuidof(*m_pd3dDevice), reinterpret_cast<void**>(&m_pd3dDevice));
    m_pd3dDevice->GetCopyableFootprints(&Desc, FirstSubresource, NumSubresources, IntermediateOffset, pLayouts, pNumRows, pRowSizesInBytes, &RequiredSize);
    m_pd3dDevice->Release();
    ...
    _In_reads_(NumSubresources) D3D12_SUBRESOURCE_DATA* pSrcData)
{
    ...
    ID3D12Device* m_pd3dDevice = nullptr;
    pDestinationResource->GetDevice(__uuidof(*m_pd3dDevice), reinterpret_cast<void**>(&m_pd3dDevice));
    m_pd3dDevice->GetCopyableFootprints(&Desc, FirstSubresource, NumSubresources, IntermediateOffset, Layouts, NumRows, RowSizesInBytes, &RequiredSize);
    m_pd3dDevice->Release();
