...
FrameResource::FrameResource(ID3D12Device* pd3dDevice, ID3D12PipelineState* pd3dScenePipelineState, ID3D12PipelineState* pd3dShadowMapPipelineState, ID3D12DescriptorHeap* pd3dDsvDescriptorHeap, ID3D12DescriptorHeap* pd3dCbvSrvDescriptorHeap, D3D12_VIEWPORT* pd3dViewport, UINT nFrameResourceIndex) :
    m_fenceValue(0),
    m_pipelineState(pd3dScenePipelineState),
    m_pipelineStateShadowMap(pd3dShadowMapPipelineState)
{
    for (UINT i = 0; i < CommandListCount; i++)
    {
        ThrowIfFailed(pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_commandAllocators[i])));
        ThrowIfFailed(pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[i].Get(), m_pipelineState.Get(), IID_PPV_ARGS(&m_commandLists[i])));
    ...
    for (UINT i = 0; i < NumContexts; i++)
    {
        ...
        ThrowIfFailed(pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_shadowCommandAllocators[i])));
        ThrowIfFailed(pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_sceneCommandAllocators[i])));

        ThrowIfFailed(pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_shadowCommandAllocators[i].Get(), m_pipelineStateShadowMap.Get(), IID_PPV_ARGS(&m_shadowCommandLists[i])));
        ThrowIfFailed(pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_sceneCommandAllocators[i].Get(), m_pipelineState.Get(), IID_PPV_ARGS(&m_sceneCommandLists[i])));
    ...
    CD3DX12_RESOURCE_DESC shadowTexDesc(
        ...
        0,
        static_cast<UINT>(pd3dViewport->Width), 
        static_cast<UINT>(pd3dViewport->Height), 
        1,
        1,
    ...
    const UINT dsvDescriptorSize = pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
    CD3DX12_CPU_DESCRIPTOR_HANDLE depthHandle(pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 1 + nFrameResourceIndex, dsvDescriptorSize); // + 1 for the shadow map.
    ...
    pd3dDevice->CreateDepthStencilView(m_shadowTexture.Get(), &depthStencilViewDesc, depthHandle);
    ...
    const UINT cbvSrvDescriptorSize = pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvStartCpuHandle(pd3dCbvSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
    CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvStartGpuHandle(pd3dCbvSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
    m_nullSrvHandle = cbvSrvStartGpuHandle;

	CD3DX12_CPU_DESCRIPTOR_HANDLE cbvSrvCpuHandle = cbvSrvStartCpuHandle;
	CD3DX12_GPU_DESCRIPTOR_HANDLE cbvSrvGpuHandle = cbvSrvStartGpuHandle;

    cbvSrvCpuHandle.Offset(nullSrvCount + textureCount + (nFrameResourceIndex), cbvSrvDescriptorSize); /**//*Error Corrected*/
    cbvSrvGpuHandle.Offset(nullSrvCount + textureCount + (nFrameResourceIndex), cbvSrvDescriptorSize); /**//*Error Corrected*/
    ...
    pd3dDevice->CreateShaderResourceView(m_shadowTexture.Get(), &shadowSrvDesc, cbvSrvCpuHandle);
    m_shadowDepthHandle = cbvSrvGpuHandle;

	cbvSrvCpuHandle = cbvSrvStartCpuHandle;
	cbvSrvGpuHandle = cbvSrvStartGpuHandle;
	cbvSrvCpuHandle.Offset(nullSrvCount + textureCount + FrameCount + (nFrameResourceIndex * 2), cbvSrvDescriptorSize);
	cbvSrvGpuHandle.Offset(nullSrvCount + textureCount + FrameCount + (nFrameResourceIndex * 2), cbvSrvDescriptorSize);
...
//    cbvSrvGpuHandle.Offset(cbvSrvDescriptorSize);
    ...
    pd3dDevice->CreateConstantBufferView(&cbvDesc, cbvSrvCpuHandle);
...
// this frame resource.
void FrameResource::WriteConstantBuffers(D3D12_VIEWPORT* pd3dViewport, Camera* pSceneCamera, Camera lightCams[NumLights], LightState lights[NumLights])
{
    ...
    pSceneCamera->Get3DViewProjMatrices(&sceneConsts.view, &sceneConsts.projection, 90.0f, pd3dViewport->Width, pd3dViewport->Height);

    // The light pass is drawn from the first light.
    lightCams[0].Get3DViewProjMatrices(&shadowConsts.view, &shadowConsts.projection, 90.0f, pd3dViewport->Width, pd3dViewport->Height);
...
void FrameResource::Bind(ID3D12GraphicsCommandList* pd3dCommandList, BOOL scenePass, D3D12_CPU_DESCRIPTOR_HANDLE* pd3dRtvCPUDescriptorHandle, D3D12_CPU_DESCRIPTOR_HANDLE* pd3dDsvCPUDescriptorHandle)
{
    if (scenePass)
    {
        // Scene pass. We use constant buf #2 and depth stencil #2
        // with rendering to the render target enabled.
        pd3dCommandList->SetGraphicsRootDescriptorTable(2, m_shadowDepthHandle);        // Set the shadow texture as an SRV.
        pd3dCommandList->SetGraphicsRootDescriptorTable(1, m_sceneCbvHandle);
        
        assert(pd3dRtvCPUDescriptorHandle != nullptr);
        assert(pd3dDsvCPUDescriptorHandle != nullptr);

        pd3dCommandList->OMSetRenderTargets(1, pd3dRtvCPUDescriptorHandle, FALSE, pd3dDsvCPUDescriptorHandle);
    }
    else
    {
        // Shadow pass. We use constant buf #1 and depth stencil #1
        // with rendering to the render target disabled.
        pd3dCommandList->SetGraphicsRootDescriptorTable(2, m_nullSrvHandle);            // Set a null SRV for the shadow texture.
        pd3dCommandList->SetGraphicsRootDescriptorTable(1, m_shadowCbvHandle);

        pd3dCommandList->OMSetRenderTargets(0, nullptr, FALSE, &m_shadowDepthView);    // No render target needed for the shadow pass.
    }
}
