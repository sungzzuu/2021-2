...
void D3D12Multithreading::LoadPipeline()
{
    ...
    // Create descriptor heaps.
    {
        ...
        const UINT cbvCount = FrameCount * 2; //FrameCount * 2
        D3D12_DESCRIPTOR_HEAP_DESC cbvSrvHeapDesc = {};
        cbvSrvHeapDesc.NumDescriptors = nullSrvCount + srvCount + cbvCount;
...
//    m_currentFrameResourceIndex = 0;
    m_pCurrentFrameResource = m_frameResources[0];
...
void D3D12Multithreading::OnUpdate()
{
    ...
    m_pCurrentFrameResource = m_frameResources[m_frameIndex];
...
void D3D12Multithreading::OnRender()
{
    ...
    {
        WCHAR cpu[64];
        swprintf_s(cpu, L"%.4f CPU Elapsed", m_cpuTime / m_titleCount);
...
void D3D12Multithreading::SetCommonPipelineState(ID3D12GraphicsCommandList* pd3dCommandList)
{
    ...
    pd3dCommandList->SetGraphicsRootDescriptorTable(3, m_samplerHeap->GetGPUDescriptorHandleForHeapStart());
