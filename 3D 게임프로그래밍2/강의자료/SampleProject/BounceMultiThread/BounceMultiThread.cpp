...
void BounceProc(void* pMyID)
{
    ...
    // Set up character & generate color attribute from thread number.
    if (*MyID > 16)
    ...
    do
    {       
        WaitForSingleObject(hScreenMutex, INFINITE); // Wait for display to be available, then lock it.
        ...
        if (!bTrails)
        {
            ...
            if ((OldCell == MyCell) && (OldAttrib == MyAttrib)) WriteConsoleOutputCharacterW(hConsoleOut, &BlankCell, 1, Old, &Dummy);
        }
    ...
    while (WaitForSingleObject(hRunMutex, 75L) == WAIT_TIMEOUT); // Repeat while RunMutex is still taken.
}
