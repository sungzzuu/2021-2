...
void CScene::BuildLightsAndMaterials()
{
	...
	m_pLights->m_pLights[0].m_xmf4Diffuse = XMFLOAT4(0.83f, 0.83f, 0.83f, 1.0f);
...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle, ID3D12Resource *pd3dDepthStencilBuffer)
{
	...
	m_pPlaneObject = new CGameObject(1);
	m_pPlaneObject->SetMesh(0, pPlaneMesh);
	m_pPlaneObject->SetMaterial(pPlaneMaterial);
	m_pPlaneObject->SetPosition(0.0f, 0.0f, 0.0f);
	CIlluminatedShader *pShader = new CIlluminatedShader();
	pShader->CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, 1, NULL, DXGI_FORMAT_D32_FLOAT);
	pShader->CreateShaderVariables(pd3dDevice, pd3dCommandList);
	m_pPlaneObject->SetShader(pShader);

	m_nShaders = 1;
	m_ppShaders = new CShader*[m_nShaders];

	CObjectsShader *pObjectShader = new CObjectsShader();
	pObjectShader->CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, 1, NULL, DXGI_FORMAT_D32_FLOAT);
	...
	m_pShadowShader = new CCircularShadowShader();
	m_pShadowShader->CreateShader(pd3dDevice, m_pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT, 1, NULL, DXGI_FORMAT_D32_FLOAT);
	m_pShadowShader->BuildObjects(pd3dDevice, pd3dCommandList, pObjectShader);
...
void CScene::ReleaseObjects()
{
	...
	if (m_pPlaneObject) delete m_pPlaneObject;
...
{
	if (m_pPlaneObject) m_pPlaneObject->ReleaseUploadBuffers();
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[0].BaseShaderRegister = 0; //Circular Shadow Texture
	...
	pd3dRootParameters[2].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[0]; //Circular Shadow Texture
...
void CScene::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	m_pPlaneObject->Render(pd3dCommandList, pCamera); //Plane
