...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CCircularShadowShader::CCircularShadowShader()
{
}
...
D3D12_INPUT_LAYOUT_DESC CCircularShadowShader::CreateInputLayout()
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC *pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_DEPTH_STENCIL_DESC CCircularShadowShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = TRUE;
...
D3D12_BLEND_DESC CCircularShadowShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
D3D12_SHADER_BYTECODE CCircularShadowShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSCircularShadow", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CCircularShadowShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSCircularShadow", "ps_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CCircularShadowShader::CreateGeometryShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GSCircularShadow", "gs_5_1", ppd3dShaderBlob));
}

void CCircularShadowShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	m_pShadowTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1);
	m_pShadowTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/CircleShadow.dds", RESOURCE_TEXTURE2D, 0);

	CObjectsShader *pObjectsShader = (CObjectsShader *)pContext;

	CreateCbvSrvDescriptorHeaps(pd3dDevice, 0, 1);
	CreateShaderResourceViews(pd3dDevice, m_pShadowTexture, 0, 2);

	CreateShaderVariables(pd3dDevice, pd3dCommandList);

	CGeometryShadowVertex* pGeometryShadowVertices = new CGeometryShadowVertex[pObjectsShader->m_nObjects];

	for (int i = 0; i < pObjectsShader->m_nObjects; i++)
	{
		XMFLOAT3 xmf3Position = pObjectsShader->m_ppObjects[i]->GetPosition();
		pGeometryShadowVertices[i].m_xmf3Position = XMFLOAT3(xmf3Position.x, 0.15f, xmf3Position.z);
		pGeometryShadowVertices[i].m_xmf2Size = XMFLOAT2(120.0f, 120.0f);
	}
	m_pGeometryShadowMesh = new CGeometryShadowMesh(pd3dDevice, pd3dCommandList, pGeometryShadowVertices, pObjectsShader->m_nObjects);

	if (pGeometryShadowVertices) delete[] pGeometryShadowVertices;
}

void CCircularShadowShader::ReleaseUploadBuffers()
{
	if (m_pGeometryShadowMesh) m_pGeometryShadowMesh->ReleaseUploadBuffers();
	if (m_pShadowTexture) m_pShadowTexture->ReleaseUploadBuffers();
}

void CCircularShadowShader::ReleaseObjects()
{
	if (m_pGeometryShadowMesh) m_pGeometryShadowMesh->Release();
	if (m_pShadowTexture) m_pShadowTexture->Release();
}

void CCircularShadowShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CShader::Render(pd3dCommandList, pCamera);

	if (m_pShadowTexture) m_pShadowTexture->UpdateShaderVariables(pd3dCommandList);
	if (m_pGeometryShadowMesh) m_pGeometryShadowMesh->Render(pd3dCommandList);
}
